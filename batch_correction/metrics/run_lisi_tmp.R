...
pcs = read.table(pca_file)
dat <- pcs[,1:(ncols(pcs)-2)]
metadata <- pcs[,ncols(pcs)-2:(ncols(pcs))]

res.df = data.frame(columns = c("methods_use", "median_iLISI", "median_cLIISI"))
for (plx in seq(10, 50, 10)){
    res <- compute_lisi(dat, metadata, colnames(metadata), perplexity = plx)
...
median_LISI(output_dir, methods_use = method_use, plx = plx, eval_metric = eval_metric)
