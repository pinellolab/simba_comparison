...
#' @param b_x_low_cutoff : Seurat FindVariableFeatures, bottom cutoff on x-axis for identifying variable genes (default 0.0125)
#' @param b_x_high_cutoff : Seurat FindVariableFeatures, top cutoff on x-axis for identifying variable genes (default 3)
#' @param b_y_cutoff : Seurat FindVariableFeatures, top cutoff on y-axis for identifying variable genes (default 0.5)
                              ...
                              batch_label = "batchlb", celltype_label = "CellType")
{
  ...
  b_seurat <- CreateSeuratObject(counts = expr_mat_seurat, meta.data = metadata, project = "Harmony_benchmark", 
  ...
  if (normData) {
    ...
    b_seurat@data = GetAssayData(b_seurat, slot = "counts")
  }

  b_seurat <- FindVariableFeatures(object = b_seurat, do.plot = TRUE, mean.function = ExpMean, dispersion.function = LogVMR, 
                                x.low.cutoff = b_x_low_cutoff, x.high.cutoff = b_x_high_cutoff, y.cutoff = b_y_cutoff)
  print(paste("Num var genes: ", length(x = VariableFeatures(b_seurat))))
  if(length(x = VariableFeatures(b_seurat)) < numVG) {
                         ...
                         visualize = T, save_obj = T)
{
  ...
  b_seurat <- RunPCA(object = b_seurat, pcs.compute = npcs, pc.genes = VariableFeatures(b_seurat), do.print = TRUE, pcs.print = 1:5, 
                      genes.print = 5)
  ...
  harmony_res <- as.data.frame(b_seurat[['harmony']]@cell.embeddings)
  cells_use <- rownames(harmony_res)

  harmony_res$batchlb <- b_seurat@meta.data$batchlb
  #str(b_seurat$meta.data$batch_label)
#  harmony_res$batch <- b_seurat@meta.data[, ]
  harmony_res$celltype <- b_seurat@meta.data$CellType
  write.table(harmony_res, file=paste0(saveout_dir,outfilename_prefix,"_Harmony_embedding_D", npcs,".txt"), quote=F, sep='\t', row.names = T, col.names = NA)
  ...
  if (visualize) {
    ...
    b_seurat <- RunTSNE(b_seurat, reduction.use = "harmony", dims = 1:npcs, k.seed = 10, do.fast = T, check_duplicates = FALSE, perplexity = tsne_perplex)
    b_seurat <- RunUMAP(b_seurat, reduction.use = "harmony", dims = 1:npcs, k.seed = 10, do.fast = T)

    ##########################################################
    #tSNE plot

    p11 <- TSNEPlot(b_seurat, pt.size = 0.5, group.by = batch_label)
    p12 <- TSNEPlot(b_seurat, pt.size = 0.5, group.by = celltype_label)
