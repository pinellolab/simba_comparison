...
src_dir = "./"
working_dir = "./Output/"
read_dir = "../data/murine_atlas/"
saveout_dir = "./Output/"
dir.create(saveout_dir, recursive = T)

expr_filename = 'filtered_total_batch1_seqwell_batch2_10x.txt.gz'
metadata_filename = 'filtered_total_sample_ext_organ_celltype_batch.txt.gz'
...
expr_mat <- read.table(file = gzfile(paste0(read_dir,expr_filename)),sep="\t",header=T,row.names=1,check.names = F)
metadata <- read.table(file = gzfile(paste0(read_dir,metadata_filename)),sep="\t",header=T,row.names=1,check.names = F)
...
batches = call_seurat3(batch_list, batch_label, celltype_label, npcs, plotout_dir = working_dir, saveout_dir = saveout_dir, outfilename_prefix = outfile_prefix, visualize = visualize, save_obj = save_obj)
